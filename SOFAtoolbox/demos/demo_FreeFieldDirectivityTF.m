...
% #Author: Michael Mihocic: robustness improved (22.07.2025)
%
...
if exist('AKtoolsStart.m', 'file')
    AKtoolsStart
    % Get the full path of the file
    fullPath = which('AKtoolsStart');

    % Get the folder containing the file
    baseFolder = fileparts(fullPath);

    % Define the subfolder to add (relative to the base folder)
    targetFolder = fullfile(baseFolder, '2_Tools');

    % Recursively add the folder and all its subfolders
