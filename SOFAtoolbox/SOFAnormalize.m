function [norm_S, param_S] = SOFAnormalize(ori_S, type, param_S)
%SOFAnormalize - Apply normalization to HRTF measurements (SOFA files).
%   Usage: [norm_S, param_S] = SOFAnormalize(ori_S, type, param_S)
%
%   SOFAnormalize supports the following normalization types (parameter 'normalization'):
...
%     param_S: (optional; normalization type 3 ('Bahu') only): structure specifying normalization parameters. Default values are:
%
...
%
%   Examples:
%     [norm_S] = SOFAnormalize(ori_S, 1)                         % -> Direction-independent filters removed individually per each receiver according to Theile (1986).
%     [norm_S, norm_param_S] = SOFAnormalize(ori_S, 3, param_S)  % -> Complex normalization to remove the specificities of the measurement sites according to Bahu et al. (2025).
%
